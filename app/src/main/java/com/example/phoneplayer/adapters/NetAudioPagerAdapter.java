...
/**
 * 作者：杨光福 on 2016/5/30 14:57
 * 微信：yangguangfu520
 * QQ号：541433511
 * 作用：xxxx
 */
public class NetAudioPagerAdapter extends BaseAdapter {
    private static final String TAG = NetAudioPagerAdapter.class.getSimpleName();
    ...
    public NetAudioPagerAdapter(Context context, List<NetAudioBean.ListEntity> lists) {
    ...
    private void bindData(int position, ViewHolder viewHolder, int itemViewType) {
        ...
        switch (itemViewType) {
            case TYPE_VIDEO://视频
                ...
                Log.e(TAG,listEntity.getVideo().getThumbnail().get(0));
                viewHolder.jz_videoplayer.setUp(listEntity.getVideo().getVideo().get(0),null);
                viewHolder.jz_videoplayer.thumbImageView.setImageURI(Uri.parse(listEntity.getVideo().getThumbnail().get(0)));
    ...
    private View initView(View convertView, ViewHolder viewHolder, int itemViewType) {
        ...
        switch (itemViewType) {
            case TYPE_VIDEO://视频
                ...
                viewHolder.jz_videoplayer =convertView.findViewById(R.id.jz_videoplayer);
                break;
            case TYPE_IMAGE://图片
